                        ...
                        <ResponsiveContainer width="100%" height="100%">
                           <BarChart data={prepareWaterfallData()} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                              ...
                              <Tooltip
                                 ...
                                 formatter={(value: number | [number, number], name: string, props: any) => {
                                    // The only series on this chart is `range`, a
                                    // [from, to] pair that draws the floating
                                    // waterfall bar. Recharts hands the raw array
                                    // to the formatter, and `[a, b].toLocaleString()`
                                    // stringifies each element and joins on a comma
                                    // — so a bar spanning [x, y] rendered as
                                    // "¥<x>,<y>", one amount visually running into
                                    // the next. The span is not the number a reader
                                    // wants anyway; the bar's own P&L is, and it is
                                    // on the datum as `value`.
                                    if (Array.isArray(value)) {
                                       const datum = props?.payload;
                                       const pl = typeof datum?.value === 'number' ? datum.value : value[1] - value[0];
                                       return [formatCurrency(pl), datum?.isTotal ? t('overview.totalLifetimePl') : t('waterfall.lifetimePl')];
                                    }
