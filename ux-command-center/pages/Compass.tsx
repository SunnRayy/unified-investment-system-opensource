...
function getAllocationRowStyle(row: AllocationRow) {
   ...
   const noTarget = row.status === 'no_target';
   if (noTarget) {
      // Nobody set a target for this class, so there is nothing to be over or
      // under. Show it as unknown rather than borrowing the "on target" green,
      // which would be just as false as the warning triangle it replaces.
      return {
         isOver: false,
         isUnder: false,
         driftValColor: 'text-slate-400',
         statusIcon: 'remove',
...
export const Compass: React.FC = () => {
   ...
   const driftingClassNames = topLevelAlloc
      .filter(r => r.status !== 'within_range' && r.status !== 'no_target')
   ...
   const buildAllocationMarkdown = () => {
      ...
      for (const row of allocation) {
         ...
         const untargeted = row.status === 'no_target';
         const drift = untargeted ? '—' : (row.drift_pct >= 0 ? '+' : '') + row.drift_pct.toFixed(1) + '%';
         const target = untargeted ? '—' : row.target_pct.toFixed(1) + '%';
         const status = untargeted ? 'no target' : row.status === 'over' ? 'OVER ▲' : row.status === 'under' ? 'UNDER ▼' : 'ok';
         lines.push(`| ${name} | ${row.current_pct.toFixed(1)}% | ${target} | ${drift} | ${status} |`);
      }
                        ...
                        {allocation.map((row, i) => {
                           ...
                           return (
                              <tr key={i} className={`border-b border-slate-200/60 dark:border-border-dark last:border-0 hover:bg-slate-50 dark:hover:bg-surface-dark transition-colors ${bgRow}`}>
                                 ...
                                 <td className={`px-5 py-2 text-right font-mono text-xs tabular-nums ${isTop ? 'font-semibold text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                                    {row.status === 'no_target' ? <span className="text-slate-400">—</span> : `${row.target_pct.toFixed(2)}%`}
                                 </td>
                                 <td className="px-5 py-2 text-right font-mono text-xs tabular-nums">
                                    <div className="inline-flex items-center gap-2.5 justify-end w-full">
                                       {row.status === 'no_target' ? (
                                          <span className="min-w-[48px] text-right font-medium text-slate-400">—</span>
                                       ) : (
                                          <>
                           ...
                           <div className="flex flex-col gap-0.5 w-full px-1">
                              {topLevelAlloc.map((r, i) => (
                                 <div key={i} className="flex items-center justify-between text-[10px] font-mono">
                                    ...
                                    <span className="font-semibold text-slate-800 dark:text-slate-200 tabular-nums">{r.status === 'no_target' ? '—' : `${r.target_pct.toFixed(1)}%`}</span>
                                 </div>
                              ))}
                           </div>
                        </div>
