...
export const performanceApi = {
    ...
    getRiskMetrics: async (includeNonRebalanceable = false): Promise<RiskMetrics> => {
        ...
        if (!res.ok) return { volatility: 0, sharpe: 0, var_95: 0, beta: 0, div_score: 0, volatility_status: "LOW", sharpe_status: "AVG", var_95_status: "LOW" };
        return res.json();
    },
    ...
    getRiskCorrelation: async (level: 'top' | 'sub' = 'top', includeNonRebalanceable = false): Promise<RiskCorrelation> => {
        ...
        if (!res.ok) return { method: '', assets: [], matrix: [] };
        return res.json();
    },
