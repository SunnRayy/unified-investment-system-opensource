...
export UIS_AUTH_TOKEN='pick-something'         # the login password — set it before the first start
./dev.sh start
```

The app requires a login and seeds the credential on first boot only. Set
`UIS_AUTH_TOKEN` beforehand and that is your password; start without it and a
random one is written to `.uis/backend.log` exactly once, as
`FIRST_BOOT_CREDENTIAL`.
...
Lifetime performance per asset — cost basis from FIFO over the real transaction
history, not a broker's running average, so a position bought across years and
partly sold still reports what it actually cost you:

![Lifetime asset performance](docs/assets/screenshot-wealthos.png)
