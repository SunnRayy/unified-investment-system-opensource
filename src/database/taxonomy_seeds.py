...
Note on `DISPLAY_MAP` (updated 2026-09-02): this tree used to be spelled out
twice more, as `DISPLAY_MAP` in `src/services/portfolio_helpers.py` and
`src/services/compass_allocation.py`. Those two were byte-identical and have
been collapsed — `portfolio_helpers` owns it now.

The remaining merge, deriving `DISPLAY_MAP` from *this* file, is **not** the
straightforward win an earlier version of this note claimed. Eight classes show
a Chinese label that differs from their `name_cn` here — 货币市场 renders as
货基, 其他贵金属 as 大宗商品, 创业投资 as 中小企业, and so on. A few are
harmless abbreviations for a narrow table column; at least two are the display
layer and the taxonomy disagreeing about what the class means. Unifying them
silently relabels eight rows in the UI, so it is an owner decision about
naming, not a refactor. The full list is in the `DISPLAY_MAP` comment.
"""
