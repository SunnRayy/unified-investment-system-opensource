...
async def get_compass_markdown(db: DatabaseConnector = Depends(get_db)):
    ...
    for r in allocation:
        ...
        # A class with no target has no drift, and printing "0.00% / +0.0%"
        # would assert a target of zero that nobody set. This table is pasted
        # into AI prompts, so a fabricated zero here becomes a premise.
        if not r.get('has_target', True):
            status_icon = "— no target"
            target_str = "—"
            drift_str = "—"
        else:
            target_str = f"{r['target_pct']:.2f}%"
            drift_str = format_signed_pct(r['drift_pct'])

        if r['is_top_level']:
            top_md += f"| {r['asset_class']} | {r['current_pct']:.2f}% | {target_str} | {drift_str} | {status_icon} |\n"
        else:
            parent_short = r['parent_class'].split(' (')[0]
            sub_md += f"| {r['asset_class']} | {parent_short} | {r['current_pct']:.2f}% | {target_str} | {drift_str} |\n"
