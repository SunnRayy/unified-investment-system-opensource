...
def count_timeline_decisions(db: DatabaseConnector) -> dict[str, int]:
    """Count the exact three populations `/decisions/timeline` renders.

    Round 2 §3: the Decision Hub showed "Total Decisions: 0" directly above a
    timeline listing real executed trades. The KPI tile was adding
    `total_insights + ai_trades_total`, and `ai_trades_total` counts only trades
    the AI can be credited with — on a portfolio whose trades came from a broker
    import, that is zero. The timeline, meanwhile, lists every trade in display
    scope. One number, two independent derivations, and the page put them next
    to each other.

    So the count lives here, next to the timeline it describes, and both
    `/timeline` and `/stats` call it. `tests/api/test_decisions_stats_match_timeline.py`
    asserts the two endpoints agree, because a shared helper only helps for as
    long as nobody writes a fourth query.

    Not limit-capped: this is a total, and the timeline's own `LIMIT` is a page
    size, not a fact about the data.
    """
    display_scope = build_trade_display_scope_sql("tl")
    insights = db.execute(
        "SELECT COUNT(*) FROM insights WHERE COALESCE(category, '') != 'lesson'"
    ).fetchone()
    drifts = db.execute("SELECT COUNT(*) FROM deviation_actions").fetchone()
    trades = db.execute(
        f"SELECT COUNT(*) FROM trade_logs tl WHERE {display_scope}"
    ).fetchone()
    counts = {
        "insight": int(insights[0]) if insights and insights[0] else 0,
        "drift": int(drifts[0]) if drifts and drifts[0] else 0,
        "trade": int(trades[0]) if trades and trades[0] else 0,
    }
    counts["total"] = counts["insight"] + counts["drift"] + counts["trade"]
    return counts
...
async def get_decisions_timeline(
    ...
    if type in ["all", "trade"]:
        ...
        # `decision_reason` and the linkage's `reason_excerpt` below are
        # reader-supplied text, rendered verbatim in every locale. A CN-fund
        # entry therefore shows `Reason: 季度分红发放` on an English screen while
        # an IBKR trade beside it reads `IBKR trade IEFA` — both correct, each in
        # the language its source wrote. This gets reported as a missed i18n
        # conversion roughly once a review; it is not one. The value is a 交易原因
        # column from the fund platform's own transaction sheet, and translating
        # it would be rewriting someone else's record. See the
        # source-language-passthrough section of docs/i18n-glossary.md.
        for r in trades:
...
async def get_decisions_stats(db: DatabaseConnector = Depends(get_db)):
    ...
    timeline_counts = count_timeline_decisions(db)
    ...
    stats = {
        ...
        # The headline tile. Counted from the same three populations the
        # timeline renders — see count_timeline_decisions().
        "total_decisions": timeline_counts["total"],
