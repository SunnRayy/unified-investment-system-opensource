...
async def get_risk_metrics(
    ...
    try:
        ...
        # Rule 12. This handler used to swallow the failure and return
        # `{volatility: 0, sharpe: 0, var_95: 0, beta: 0, div_score: 0}` with
        # HTTP 200 — a fabricated payload indistinguishable from a real
        # measurement. That is how Risk Matrix came to show five hard zeros on
        # a fully-synced portfolio and nobody could tell it had failed: beta 0
        # and div_score 0 are not producible by calculate_portfolio_risk() at
        # all (an empty portfolio yields beta 0.8, div_score 2), so the zeros
        # were only ever reachable through this line.
        #
        # The page renders "--" whenever it has no metrics, so a real error
        # status is both honest and better-looking than the lie was.
        logger.exception("risk metrics failed")
        return api_error_response(e, context="risk_metrics")
