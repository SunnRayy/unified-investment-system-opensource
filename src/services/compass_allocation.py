...
# DELIBERATELY BILINGUAL — owner ruling, 2026-08-30. Renders both languages at once
# regardless of UI locale; that is intended for asset-class terminology, not a missed
# i18n conversion. Full rationale, and the duplication warning, live on the identical
# copy in src/services/portfolio_helpers.py — keep the two in sync.
DISPLAY_MAP = {
    "Equity": "Equity (股票)",
...
def build_compass_allocation(
    ...
    for top_key in all_top_keys:
        ...
        for sub_key, sub_data in sorted_children:
            ...
            if abs(sub_drift_pct) > sub_target_info["tolerance"]:
