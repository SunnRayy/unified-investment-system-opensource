...
export UIS_AUTH_TOKEN='pick-something'         # 登录密码——首次启动前先设好
./dev.sh start
```

应用需要登录，而且只在首次启动时写入凭据。事先设好 `UIS_AUTH_TOKEN`，它就是
你的密码；如果没设就启动，系统会生成一个随机密码，并且只往 `.uis/backend.log`
里写一次，标记为 `FIRST_BOOT_CREDENTIAL`。
