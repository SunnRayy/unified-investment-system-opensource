...
class TestGetBenchmarkProxyCodes:
    ...
    def test_committed_example_config_yields_a_usable_proxy_list(self):
        """The config a fresh clone actually gets must resolve to real codes.

        This replaces a test that read whatever `config/settings.yaml` happened
        to be on disk and asserted the 6-code historical default. That was
        green on one machine and red everywhere else: settings.yaml is
        gitignored, so CI and every clean clone fall back to
        settings.example.yaml, which ships four generic codes rather than the
        owner's two real fund codes. It was also redundant — the owner's real
        settings.yaml has no `benchmark_proxy_codes` key at all, so on his
        machine it exercised the same default path as the hermetic test above.

        Reading the example file by name keeps this honest: it asserts what a
        new user is handed, not what one developer's untracked file contains.
        """
        import yaml
        ...
        example = yaml.safe_load(
            (REPO_ROOT / "config" / "settings.example.yaml").read_text(encoding="utf-8")
        )
        codes = example["verification"]["benchmark_proxy_codes"]

        assert codes, "settings.example.yaml must ship a non-empty proxy list"
        with patch("src.config.load_config", return_value=example):
            assert get_benchmark_proxy_codes() == tuple(codes)
