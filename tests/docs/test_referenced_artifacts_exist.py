...
# plus gitignored files that legitimately do not exist in a clean checkout.
ALLOWLIST: set[str] = {
    ...
    # Gitignored by design (owner-specific paths and prompts), so it is absent
    # from every fresh clone and from CI. The docs must still be able to name
    # it — it is the file a new user creates from settings.example.yaml. The
    # reference is real; the file's absence is the point.
